                    ...
                    <div class="card-body table-responsive px-4  pt-4" >
                        <table class="table table-bordered table-hover text-nowrap" id="data-table">
                            <thead>
                                <tr>
                                    <th>ID</th>
                                    <th style="width: 37rem;">Department</th>
...
<div class="modal fade" id="editModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ url('/Department-Group/Edit') }}" method="POST" id="updateDepartmentForm">
                @csrf
                @method('PUT')
...
<script>
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $.each(response.data, function(index, item) {
                    let statusSelect =
                        `<select class="form-select select-status" data-id="${item.dep_id}">
                            <option value="1" ${item.status == 1 ? 'selected' : ''}>Active</option>
                            <option value="0" ${item.status == 0 ? 'selected' : ''}>Inactive</option>
                        </select>`;
                    let editButton =
                        `<button class="btn btn-primary btn-sm editbtn me-2" data-department-id="${item.dep_id}"><i class="fas fa-edit"></i></button>`;
                    let deleteButton =
                        `<button class="btn btn-danger btn-sm deletebtn" data-department-id="${item.dep_id}"><i class="fas fa-trash-alt"></i></button>`;
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.editbtn', function() {
            ...
            $.ajax({
                type: 'GET',
                url: `/Department-Group/Select/${dep_id}`,
                success: function(response) {
        ...
        $(document).on('click', '.deletebtn', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    $('#group_name').val(response.data.group_name);
                    $('.modal fade').html(response.data.group_name)
        ...
        $('#myModaldel').on('click', '#delete-confirm-button', function() {
            ...
            $.ajax({
                type: 'DELETE',
                url: `/Department-Group/Remove/${dep_id}`,
                headers: {
        ...
        $('#data-table').on('change', '.select-status', function() {
            ...
            $.ajax({
                type: 'PATCH',
                url: `/Department-Group/EditStatus/${updateId}`,
                headers: {
                    'X-CSRF-TOKEN': csrfToken
                },
                data: {
