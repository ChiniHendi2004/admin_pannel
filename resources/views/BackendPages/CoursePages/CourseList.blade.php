...
<script>
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $.each(response.data, function(index, item) {
                    var addDetails =
                        `<a href="/Course-Details/Page/${item.id}" class="btn text-white" style="background-color: #00008B">Add Details</a>`;
                    var viewCourse =
                        `<a href="/View-Course/Page/${item.id}" class="btn py-2 mr-2 rounded  mx-1 " style="background-color: #16641b; font-size: 13px;"><i class="far fa-eye text-white"></i></a>`;
                    var editButton =
                        `<a href="/Edit-Course/${item.id}" class="btn py-2 mr-2 rounded editbtn text-white" style="background-color: #00008B; font-size: 13px;"><i class="fas fa-edit text-white"></i></a>`;
                    var deleteButton =
    ...
    $('#myModaldel').on('click', '#delete-confirm-button', function() {
        $.ajax({
            type: "DELETE",
            url: "/Delete/Course/" + course_id, // Fixed URL
            headers: {
