...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Committee Groups</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="#">Home</a></li>
                        <li class="breadcrumb-item active">Committee Groups</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editModalLabel">Edit Committee Group</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="editCommitteeGroupForm">
                    @csrf
...
<div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="deleteModalLabel">Delete Committee Group</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <p>Are you sure you want to delete this Committee group?</p>
            </div>
            <div class="modal-footer">
...
<script>
    $(document).ready(function() {
        ...
        function fetchCommitteeGroups() {
            $.ajax({
                url: "{{ url('/Committee/Group/List') }}",
                type: 'GET',
                dataType: 'json',
                ...
                error: function() {
                    alert('Error fetching committee groups.');
                }
            });
        }

        // Initial fetch of committee groups
        ...
        $('#saveEditButton').click(function() {
            var formData = $('#editCommitteeGroupForm').serialize();
            var groupId = $('#editGroupId').val();

            $.ajax({
                url: `/Edit/Committee/Group/${groupId}`,
                type: 'PUT',
                data: formData,
                success: function(response) {
                ...
                error: function() {
                    alert('Error updating committee group.');
                }
            });
        });
        ...
        $('#confirmDeleteButton').click(function() {
            ...
            if (groupIdToDelete) {
                $.ajax({
                    url: `/Delete/Committee/Group/${groupIdToDelete}`,
                    type: 'DELETE',
                    headers: {
                    ...
                    error: function() {
                        alert('Error deleting committee group.');
                    }
                });
            }
        });

        // Form submission to add a new committee group
        $('#committeeGroupForm').submit(function(e) {
            e.preventDefault();
            var formData = $(this).serialize();

            // Optionally, add extra data if needed (like tenant_id, etc.)
            formData += '&tenant_id=1&client_slug=clint-part&website_url=www.col.com&employee_id=1&academic_session=2025&expiration_date=2025/12/12&status=1';

            $.ajax({
                url: "{{ url('/Create/Committee/Group') }}",
                type: 'POST',
                data: formData,
                success: function(response) {
                    $('#responseMessage').html(`<div class="alert alert-success">${response.message}</div>
                    `);
                    $('#committeeGroupForm')[0].reset();
                    fetchCommitteeGroups();
                },
                error: function() {
                    alert('Error creating committee group.');
                }
            });
        });
    });
